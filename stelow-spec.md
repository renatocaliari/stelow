...
**Files:**
- `stelow.json` — Project-local workflow tracking
- `.stelow-global.json` — Global cross-project workflows
- `.stelow/{date}/{slug}/` — Per-workflow artifacts
...
|---------|-------|---------|
| `/sw-start` | `/sw-start` | Start new workflow |
| `/sw-abort` | `/sw-abort` | Abort and archive workflow(s) — kill active, keep disk copy |
| `/sw-pause` | `/sw-pause` | Pause active workflow |
| `/sw-resume` | `/sw-resume` | Resume paused workflow |
| `/sw-next` | `/sw-next` | Advance to next phase |
| `/sw-goto` | `/sw-goto` | Jump to specific phase |
| `/sw-rename` | `/sw-rename` | Rename workflow |
| `/sw-menu` | `/sw-menu` | Show workflow menu |
| `/sw-archive` | `/sw-archive` | Archive workflow(s) |
| `/sw-unarchive` | `/sw-unarchive` | Restore archived workflow |
...
| dirHash | string | Stable directory name (e.g., `sw-ollc-whkaxv`) |
...
Save to .stelow/{date}/{dir}/plans/critique-report.md
```
...
Also reference cali-product-coding-standards for coding principles + Datastar framework philosophy.
...
Input: .stelow/{date}/{dir}/plans/spec-product_{v}.md
Output: .stelow/{date}/{dir}/plans/spec-tech_{v}.md
```
...
BR-019: Bypass flag cleared when agent advances to Phase 10 (Selection) or beyond via /sw-next.
...
❓ **UNCLEAR:** The global tracking file (`.stelow-global.json`) appears designed for cross-project workflow visibility but the full synchronization logic was not traced.