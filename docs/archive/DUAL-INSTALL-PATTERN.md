...
This document analyzes the dual-install pattern used by context-mode to understand how to implement a similar pattern for stelow. The pattern allows a package to be installed both as a global CLI tool (via npm) and as a pi extension (via pi install).
...
The dual-install pattern is well-proven by context-mode (14K+ stars, used at Microsoft, Google, Meta, etc.). For stelow to integrate with pi as both a CLI tool and an extension, following this pattern is recommended.
